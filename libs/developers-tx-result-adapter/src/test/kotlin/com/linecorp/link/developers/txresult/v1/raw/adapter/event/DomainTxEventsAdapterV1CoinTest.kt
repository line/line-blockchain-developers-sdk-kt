...
class DomainTxEventsAdapterV1CoinTest {
    ...
    @Test
    fun `given coin_transfer tx-result, then extract 'EventCoinTransferred' `() {
        val rawTransactionInJsonText =
            RawTransactionLoader.loadRawTransactionResultInJsonText("raw-transaction/coin_transfer.json")
        ...
        assert(actual.filterIsInstance<EventCoinTransferred>().isNotEmpty())
        val eventCoinTransferred = actual.first() as EventCoinTransferred
        assertEquals("tcony", eventCoinTransferred.denomination)
        assertEquals("1", eventCoinTransferred.amount)
    }
}
